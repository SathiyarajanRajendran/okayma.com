...
export function sendTestimonialConfirmEmail(env, { to, name, link }) {
  return send(env, {
    to,
    subject: "Confirm your testimonial for Okayma",
    heading: `Thank you, ${name}`,
    body: "Confirm this address and your testimonial goes to Sathiya for review. Nothing is published until you confirm and he approves it, and your email address is never shown on the site.",
    actionLabel: "Confirm my testimonial",
    actionUrl: link,
    footer: "This link works once and expires in 24 hours. If you did not write a testimonial for Okayma, ignore this email and nothing is published.",
  });
}

export function sendTestimonialDecisionEmail(env, { to, name, published, siteUrl }) {
  return send(env, {
    to,
    subject: published ? "Your testimonial is now live on Okayma" : "An update on your testimonial",
    heading: published ? "Your testimonial has been published" : "Your testimonial was not published",
    body: published
      ? `${name}, thank you — your words are now on the Okayma testimonials page, credited to you.`
      : `${name}, thank you for writing. Your testimonial was read but has not been published.`,
    actionLabel: "Open the testimonials page",
    actionUrl: `${siteUrl}/testimonials`,
    footer: "If you would like anything changed or removed, reply to this address and it will be done.",
  });
}
